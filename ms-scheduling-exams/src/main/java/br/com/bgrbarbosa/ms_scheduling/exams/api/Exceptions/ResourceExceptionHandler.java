package br.com.bgrbarbosa.ms_scheduling.exams.api.Exceptions;



import br.com.bgrbarbosa.ms_scheduling.exams.config.Messages;
import br.com.bgrbarbosa.ms_scheduling.exams.infraestruture.exceptions.DatabaseException;
...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> LaudoNotFoundException(LaudoNotFoundException e, HttpServletRequest request) {
		HttpStatus status = HttpStatus.BAD_REQUEST;
		StandardError err = new StandardError();
		err.setTimestamp(Instant.now());
		err.setStatus(422);
		err.setError("Report not attached.");
		err.setMessage(Messages.REPORT_NOT_ATTACHED);
		err.setPath(request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}

	@ExceptionHandler(UserException.class)
	public ResponseEntity<StandardError> loginException(UserException e, HttpServletRequest request) {
		HttpStatus status = HttpStatus.BAD_REQUEST;
		StandardError err = new StandardError();
		err.setTimestamp(Instant.now());
		err.setStatus(403);
		err.setError("Access permission error!");
		err.setMessage(Messages.FORBIDEN);
		err.setPath(request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
